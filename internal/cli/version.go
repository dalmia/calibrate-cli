...
// which propagates the value here (see cmd/calibrate/main.go):
//
//	go build -ldflags "-X main.version=x.y.z" ./cmd/calibrate
var Version = "0.0.13"
...
func initVersionCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Long: `Print the current version of the calibrate CLI.
...
overridden at build time using Go linker flags:
  ...
  go build -ldflags "-X main.version=x.y.z -X main.buildTime=$(date -u +%Y-%m-%dT%H:%M:%SZ)" ./cmd/calibrate`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if _, err := fmt.Fprintf(cmd.OutOrStdout(), "calibrate %s\n", Version); err != nil {
				return err
			}
			if BuildTime != "" {
