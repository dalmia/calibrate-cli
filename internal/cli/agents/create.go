...
var createCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "interaction-type", Shorthand: "i", FieldPath: "Body.InteractionType", Kind: flagutil.FlagKindEnum, Optional: true, HasDefault: true, DefaultStr: "conversation", EnumValues: []string{"conversation", "general"}, Description: "What the agent expects in the request body:\n\n- `conversation`: a normal back-and-forth agent, answers within an ongoing conversation. Receives `{\"messages\": [...]}`\n- `general`: a one-shot agent, takes a single plain input and produces a single plain output, no conversation. Receives `{\"input\": \"...\"}` (options: conversation, general)"},
